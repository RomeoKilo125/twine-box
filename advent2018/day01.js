...
function findRepeatedFrequency(start, changes) {
  for (let i = 0; i < changes.length; i++) {
    const newFreq = freqLog[i + start] + changes[i]
    freqLog.push(newFreq)
    if (freqCount[newFreq]) {
      freqCount[newFreq]++
      return newFreq
    } else {
      freqCount[newFreq] = 1
  ...
  return findRepeatedFrequency(freqLog.length - 1, changes)
}

console.log('first repeat ' + findRepeatedFrequency(freqLog.length - 1, input))
