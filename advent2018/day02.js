...
console.log(exactlyTwo.length * exactlyThree.length)

// Part Two
/**
 * a function that takes in two strings and returns the number of mismatching characters
 *
 * @param {string} str1 string1
 * @param {string} str2 string2
 * @returns {number} the count of mismatches between the two strings
 */
function countMismatches (str1, str2) {
  if (str1.length !== str2.length) {
    return 'Length Disparity'
  }
  let count = 0
...
/**
 * a function to take in two strings and return pairs of them that have exactly one mismatch between them
 *
 * @param {Array} arr an array of strings
 * @returns {Array} an array of arrays each with two elements
 */
function findSimilarIDs (arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    for (let j = i + 1; j < arr.length; j++) {
      const test2 = arr[j]
      if (countMismatches(test1, test2) === 1) {
        pairs.push([test1, test2])
      }
    }
  }
  return pairs
...
console.log(findSimilarIDs(input))
